...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  Step        :  Initial Training: Train Random Forest
...
def get_inference_pipeline(rf_config, max_tfidf_features):
    """
    To generate a inference pipeline for provided model and feautres 
    """
    ...
    # Build a pipeline with SimpleImputer & OneHotEncoder:
    non_ordinal_categorical_preproc = make_pipeline(
        SimpleImputer(strategy='most_frequent'),
        OneHotEncoder()
    )
    
    # Impute the numerical columns to handle missing values
    # NOTE: RF algorithm does not require scalling
    zero_imputed = [
        "minimum_nights",
        "number_of_reviews",
        "reviews_per_month",
    ...
    sk_pipe = Pipeline([
        ('features_preprocessor', preprocessor),
...
def plot_feature_importance(pipe, feat_names):
    """
    To generate feature importance plot for provided model and feautres 
    """
...
def go(args):
    """
    main function to orchestrate the operations of the pipeline
    """
    ...
    X_train, X_val, y_train, y_val = train_test_split(
        X, y, test_size=args.val_size, stratify=X[args.stratify_by], random_state=args.random_state
    )

    sk_pipe, processed_features = get_inference_pipeline(rf_config, args.max_tfidf_features)
    logger.info("Success: Preparing sklearn pipeline")

  
    # Fit the pipeline sk_pipe by calling the .fit method on X_train and y_train
    sk_pipe.fit(X_train, y_train)
    logger.info("Success: Fitting sklearn pipeline")
    ...
    logger.info("Success: Scoring sklearn pipeline")
    ...
    if os.path.exists("models/random_forest_dir"):
        shutil.rmtree("models/random_forest_dir")

    # Save the sk_pipe pipeline as a mlflow.sklearn model in the directory "models/random_forest_dir"
    ...
    artifact.add_dir("models/random_forest_dir")
    run.log_artifact(artifact)
    logger.info("Success: Uploading model artifact to W&B")
    ...
    # save r_squared under the "r2" key & "mae" under "mae" key
    run.summary['r2'] = r_squared
    run.summary['mae'] = mae
    logger.info("Success: Logging metrics and losses")
    ...
    logger.info("Success: Uploading to W&B the feture importance visualization")
